...
def main(img1, img2):
    ...
    img2 = cv2.imread("C:/Abhi/XAMPP/htdocs/FindOURS/FINDOURS/fingerprint_recognition/fingerprint-recognition/db2/" + img2, cv2.IMREAD_GRAYSCALE)
...
def matcher():
    path_to_database = "C:/Abhi/XAMPP/htdocs/FindOURS/FINDOURS/fingerprint_recognition/fingerprint-recognition/database"
    path_to_db2 = "C:/Abhi/XAMPP/htdocs/FindOURS/FINDOURS/fingerprint_recognition/fingerprint-recognition/db2/"
    img_list = [img for img in os.listdir(path_to_database) if os.path.isfile(os.path.join(path_to_database, img))]    
    db2_img = [db2img for db2img in os.listdir(path_to_db2) if os.path.isfile(os.path.join(path_to_db2, db2img))]

    for img_path in img_list:
        for db2 in db2_img:
            hasMatched, matched_img = main(img_path, db2)

            if hasMatched:
                # print(matched_img["name"])
                # print(hasMatched)
                print(img_path, " :matches with: ", db2)
